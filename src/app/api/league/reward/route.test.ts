...
const mocks = vi.hoisted(() => ({
    ...
    notify: vi.fn(), redeemPoints: vi.fn(), updateStanding: vi.fn(),
}));
...
const award = { id: "weekly-award", userId: "champion", awardType: "WEEKLY_CHAMPION", status: "AVAILABLE" };
function chooseCash() {
    return POST(new NextRequest("http://localhost/api/league/reward", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ awardId: award.id, rewardCode: "CASH_300" }),
    }));
}
...
describe("weekly champion cash reward", () => {
    ...
    it("lets an existing weekly champion choose cash without spending RP or changing league points", async () => {
        ...
        expect(mocks.findAward).toHaveBeenCalledWith({ where: { id: award.id, userId: "champion" } });
