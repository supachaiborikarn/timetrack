...
- กันส่งซ้ำ (ดูข้อ 5.1) → ตอบ 409 พร้อม refCode เดิมและเหตุผล
- จำกัด 5 ไฟล์/ใบสมัคร, รูปถ่าย ≤ 2MB, เอกสาร ≤ 5MB

### 5.1) การกันใบสมัครซ้ำ

**ปัญหาที่เจอจริง (18 ส.ค. 2569):** เกณฑ์เดิมคือ "เบอร์เดิม **และ** ชื่อตำแหน่งเดิม ภายใน 30 วัน"
พอเปิดใช้ระบบประกาศรับสมัคร (`JobOpening`) ชื่อตำแหน่งเปลี่ยนจากที่ผู้สมัครพิมพ์เอง ("พนักงานปั้ม")
เป็นชื่อจากประกาศ ("พนักงานหน้าลานปั๊มน้ำมัน") เงื่อนไขสองอันเลยไม่ตรงพร้อมกัน มีคนสมัครซ้ำหลุดเข้ามา 3 คน
(APP-69-0009/0022, 0012/0020, 0013/0023) ทั้งที่เป็นคนเดียวกัน เบอร์เดียวกัน เลขบัตรเดียวกัน

**เกณฑ์ใหม่ — ดูที่ "ตัวคน" ไม่ใช่ตำแหน่ง:**

| ตัวจับคู่ | เหตุผล |
|---|---|
| `citizenIdHash` ตรงกัน | เลขบัตรประชาชนคือตัวระบุตัวตนที่เชื่อถือได้ที่สุด |
| `phone` + `firstName` + `lastName` ตรงกันครบ 3 อย่าง | เผื่อกรณีกรอกเลขบัตรผิด — ไม่ใช้เบอร์อย่างเดียวเพราะคนในบ้านเดียวกันใช้เบอร์ร่วมกันได้ |

เมื่อเจอใบเดิม (ดูใบล่าสุด) จะตัดสินตามสถานะ:

| สถานะใบเดิม | ผล | ข้อความ |
|---|---|---|
| `SUBMITTED` / `SCREENING` / `INTERVIEW` / `OFFERED` | **บล็อก** ไม่จำกัดเวลา | "คุณมีใบสมัครที่อยู่ระหว่างการพิจารณาอยู่แล้ว (ตำแหน่ง…)" |
| `HIRED` | **บล็อก** | "ข้อมูลนี้ได้รับการจ้างงานเรียบร้อยแล้ว" |
| `REJECTED` | **บล็อก 30 วัน** นับจากวันที่ยื่น | "สมัครใหม่ได้อีกครั้งในอีก N วัน" |
| `WITHDRAWN` | **ผ่าน** | ผู้สมัครถอนเอง มักเพราะกรอกผิดแล้วอยากกรอกใหม่ |
| `DRAFT` | ไม่นับ | |

**ทำไมต้องมีคอลัมน์ `citizenIdHash`:** `citizenIdEnc` ใช้ AES-GCM ที่ IV สุ่มทุกครั้ง เลขเดิมจึงได้ ciphertext
ไม่ซ้ำกัน ค้นด้วย query ไม่ได้เลย จึงเพิ่มคอลัมน์ HMAC-SHA256 (คีย์เดียวกับ `FIELD_ENCRYPTION_KEY`) ที่ค่าคงที่
สำหรับ input เดิม — ใช้เทียบเท่ากันได้แต่ย้อนกลับเป็นเลขบัตรไม่ได้ และเพราะเป็น HMAC ที่มีคีย์
ต่อให้ DB หลุดก็ brute-force เทียบกับเลขบัตร 13 หลักทั้งหมดไม่ได้ (`hashFieldForLookup()` ใน `src/lib/crypto-field.ts`)
ใบสมัครเดิม 25 ใบ backfill ค่านี้ครบแล้ว
...
- ~~ตอนกดจ้างงาน รูปโปรไฟล์**ไม่ย้าย/rename**ไฟล์บน Cloudinary แต่ให้ `User.photoUrl` ชี้ไปที่ Cloudinary key เดียวกับไฟล์แนบของใบสมัคร~~ **เปลี่ยนแล้ว (19 ส.ค. 2569)** — วิธีเดิมมีปัญหาสองชั้น: (1) `photoUrl` เก็บ public_id ดิบ แต่ UI เอาไปใส่ `<AvatarImage src>` ตรง ๆ รูปพนักงานที่จ้างมาจึง**ไม่เคยแสดงเลย** ตกไปที่ตัวอักษรย่อทุกครั้ง และ (2) การใช้ Cloudinary object ร่วมกันกลายเป็นกับดักทันทีที่มีฟีเจอร์เปลี่ยนรูปพนักงาน — ลบรูปเก่าเมื่อไหร่ ไฟล์แนบของใบสมัครที่ยังชี้ไปที่ object เดิมจะพัง ตอนนี้ตอนจ้างจะ**คัดลอก**ไฟล์แนบทั้งหมดเข้าคลังเอกสารพนักงาน และ `photoUrl` ชี้ไปที่ `/api/employees/<id>/photo` — ดูรายละเอียดใน [employee-media-design.md](./employee-media-design.md)
...
- **บั๊กที่เจอในโค้ดตัวเอง (18 ส.ค. 2569):** route จ้างงานเดิมสร้าง `User` แล้วค่อยอัปเดตใบสมัครแยกกันคนละคำสั่ง ถ้าคำสั่งหลังพลาดจะได้ใบสมัครสถานะ `HIRED` ที่ `hiredUserId` ว่าง — สถานะที่ลบก็ไม่ได้ แก้ผ่าน UI ก็ไม่ได้ (เกิดจริงกับ APP-69-0012) แก้แล้วด้วย interactive transaction และเพิ่มปุ่ม "ยกเลิกการจ้าง" ที่ย้อนสถานะกลับเป็น `REJECTED` เพื่อกู้เคสแบบนี้ได้
- **ปุ่มยกเลิกการจ้าง:** `POST /api/admin/applications/[id]/unhire` (permission `application.hire`, ต้องระบุเหตุผล) — `src/lib/employee-removal.ts` จะไล่นับข้อมูลใน 24 ตาราง ถ้าบัญชีพนักงาน**ยังไม่เคยถูกใช้เลย**จะลบทิ้งจริง ถ้าเคยใช้แล้ว (มีเวลาเข้างาน/เงินเดือน/ฯลฯ) จะแค่ปิดใช้งาน `isActive: false` + `employeeStatus: RESIGNED` เพื่อไม่ให้ FK cascade ลบข้อมูลเวลาเข้างานทิ้ง
- **บั๊กร้ายแรงที่ทำให้ส่งใบสมัครไม่ได้ทั้งระบบ (18 ส.ค. 2569):** `generateRefCode()` เดิมออกเลขจาก `count() + 1` ซึ่งถูกต้องตราบใดที่ไม่มีการลบใบสมัคร พอลบใบสมัครทิ้ง count ลดลง ระบบจึงออกเลขที่ใบอื่นถืออยู่แล้ว ชน unique constraint ของ `refCode` และเพราะ count ไม่เปลี่ยนเมื่อ insert ล้มเหลว ลูป retry 5 รอบจึงได้เลขเดิมทุกรอบ → ตอบ 500 "ส่งใบสมัครไม่สำเร็จ" **ทุกคนสมัครไม่ได้เลยจนกว่าจะแก้** เกิดจริงหลังลบใบซ้ำ 4 ใบ มีผู้สมัคร 1 รายกรอกจนจบแล้วส่งไม่ได้ แก้เป็นออกเลขจาก**ค่าสูงสุดที่เคยออก** เลขที่ว่างจากการลบจะไม่ถูกนำกลับมาใช้ซ้ำ ย้ายไปไว้ที่ `src/lib/application-ref-code.ts` พร้อมเทสต์ 8 เคส
